...
## 55スライドを2時間で実施する場合の検討

PPT が55ページになった場合の時間配分と実施可否の目安です。

### 結論：**条件付きで2時間で可能**

- **環境が事前に整っている**（WSL/Docker 起動済み・権限済み）なら、必須3API（Step0〜3）＋説明を2時間に収められる。
- **Step4（報酬）は「余力があれば」にし、時間が足りなければ省略**する前提にすると余裕ができる。
- ボトルネックは **Step2（チケット作成・AI実装）**。ここで詰まると一気に時間を消費する。

### 推奨時間配分（合計 120 分）

| ブロック | 内容 | 目安時間 | 備考 |
|----------|------|----------|------|
| オープニング・全体像 | 本日の流れ、アーキテクチャ、進め方 | 15分 | 55枚は「参照用」とし、説明は要点のみ（5〜10枚程度を口頭でカバー） |
| Step0 | 環境確認・healthz・Swagger | 10分 | 事前準備済みなら「確認だけ」で短縮 |
| Step1 | アカウント作成・ログイン・APIの型 | 10分 | Swagger 操作の習熟 |
| **Step2** | **チケット作成（AIで実装）** | **35分** | プロンプト実行→確認→不具合時はTAサポート。ここを守る |
| Step3 | チェックイン（GET/POST・初回/再実行） | 15分 | Step2の token を使う流れ |
| Step4 | 報酬（reward/create・list・checkin報酬） | 15分 | **時間がなければ省略または5分で要点のみ** |
| まとめ・Goal判定・次の演習案内 | スライド最後＋質疑 | 20分 | バッファ兼ねる |

### 実施のコツ

1. **スライドは「全部説明しない」**  
   55枚は資料として配布し、説明は「今日使う部分」に絞る（例: Step0〜4の手順スライド＋詰まりポイントのみ）。
2. **Step2で時間を取られすぎないようにする**  
   - 事前に「仕様書（041）とプロンプトを開いておく」「Plan/Agent の使い方」を1枚で案内する。  
   - 実装が一発で動かない場合の「切り札」として、goal ブランチの該当箇所を参照させる or TAが差分ヒントを渡す。
3. **Step4を省略する場合**  
   - 「報酬は時間があれば」「資料の〇〇ページ以降を各自で」と案内し、必須は Step0〜3＋まとめにすると 2 時間で収まりやすい。
4. **開始前に確認**  
   - 「Docker 起動できている人」「Swagger が開ける人」で挙手してもらい、未準備の人が多ければ Step0 を 15 分に延長し、Step4 を短縮または省略。

### リスクと対策

| リスク | 対策 |
|--------|------|
| Step2 で AI が思うように動かない | 事前に goal の実装を開いておく／TA が views.py の差分を共有 |
| 環境で詰まる（docker.sock 等） | Step0 の「よくある詰まり」スライドを 1 枚用意し、権限・127.0.0.1 を口頭で補足 |
| 55枚を端から説明して時間切れ | 説明は「本日の流れ＋各Stepの成功条件」に限定し、詳細は資料参照とする |
...
**削除するファイル**（032 以降の演習不要分、13件）を**削除して問題が少ない順**に並べる。
