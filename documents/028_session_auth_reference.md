...
チケット拡張API（ticket_system）におけるセッションとベースAPI認証の扱いをまとめる。

## 学習での位置づけ（v1-start）

- この資料は「実装中に認証で詰まったとき」に読むリファレンス
- まずは `documents/011_goal_steps_ai_pairing.md` を先に進め、必要なタイミングで戻って参照する
- Goal到達の観点では、特に「Cookie取得順」と「ログイン/ログアウトの流れ」を理解できれば十分
...
Cookieの状態: ...
セッションの状態: ...
この資料の観点で、どの順に切り分けるべきか教えてください。
```

## 1. 概要

- **拡張API**: ticket_system（例: `https://ticket.buxbit.net`）。チケット画像・チェックイン・NFT作成などを行う。
- **ベースAPI**: 認証・ユーザー情報・NFT メタデータなどを提供するAPI（例: `https://linode.buxbit.net`）。
