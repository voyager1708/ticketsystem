...
実装は `010_Step0_Setup.md` から `010_Step4_Reward.md` のStep順で進め、仕様確認が必要になった時点で参照してください。
...
現在のリクエスト: ...
現在のレスポンス: ...
040_ticket_create_api_spec.md と照らして、差分と修正方針を教えてください。
```
...
sequenceDiagram
    ...
    Note right of Client: event_name, event_date, ticket_html (必須)<br/>venue, seat, recipient_paymail (任意)
...
**Content-Type は `multipart/form-data` のみ対応**（JSON ボディは受け付けない）。必須: `event_name`, `event_date`, `ticket_html`（HTML ファイル）。

### 5.2 リクエスト例（multipart/form-data）
...
curl -X POST 'https://ticket.buxbit.net/api/ext/v1/ticket/create' \
  ...
  -F 'ticket_html=@/path/to/ticket.html'
```

### 5.3 成功レスポンス（201 Created）
...
他例: `"event_date is required"`, `"ticket_html is required (HTML file with styles)"`, `"ticket_html must be UTF-8 text"`。`render_ticket_html_for_creation` 内でバリデーションエラーになった場合も 400 で `error` を返す。
...
セッション未ログイン時（base_api_cookies も sessionid も無い場合）に返す。
...
HTML レンダリング失敗時は `"error": "Failed to render ticket HTML"` を返す。
...
`holder_paymail` の設定は `app/ticket_service/views.py` の `TicketCreateAPIView`（`recipient_paymail` を `holder_paymail` に渡す処理）と `app/ticket_service/services/base_api_client.py` の `create_ticket_nft` を参照。
...
現行実装では管理者限定オプションは未実装。**ログイン済みユーザー（セッションあり）は誰でもチケットNFTを作成可能**。権限制御を追加する場合は `TicketCreateAPIView.post` 内で `request.user.is_staff` 等を参照して実装する。
