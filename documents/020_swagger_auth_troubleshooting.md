...
この資料は、生徒が `v1-start` で実装を進める際に、Swagger認証まわりで詰まったときのトラブルシュート集です。  
最初から通読するより、問題が起きた章だけ参照する使い方を推奨します。

> **注**: 本プロジェクトは **drf-spectacular** を使用しています（旧 drf-yasg から移行済み）。  
> 「1. カスタムSwagger UIテンプレート」は drf-yasg 時代の記述です。drf-spectacular の Swagger UI は標準でセッション認証をサポートするため、カスタムテンプレートは不要です。セッション・Cookie の挙動（2章以降）は引き続き参考になります。
...
症状: ...
再現手順: ...
関連箇所: 020_fix_points の「X章」
この章の内容を前提に、最小修正案を1つ提案してください。
```

## 概要

`ticket_system`のSwagger UIでベースAPI経由のログイン/ログアウト機能を実装する際に発生した問題と、その解決策をまとめます。

## 環境

- **ticket_system**: 拡張APIサーバー（`https://ticket.buxbit.net`）
- **ベースAPI**: 認証・NFT等を提供するAPIサーバー（例: `https://linode.buxbit.net`）
