...
- 認証や環境変数で詰まったときは、実装（`app/ticket_service/views.py` のログイン・セッション、`base_api_client.py` のクッキー渡し）を参照する
- 実装詳細は `app/ticket_service/services/base_api_client.py` を参照
...
- チケット作成・チェックイン・画像取得などでは、この保存済みクッキーをベースAPI に付けて呼び出す（実装は `views.py` の `_extract_session_cookies` や `base_api_cookies` を参照）
...
| 資料 | 内容 |
|------|------|
| `010_Step0_Setup.md` 以降の各 Step 資料 | 実行ステップと必須3API |
| `040_ticket_create_api_spec.md` | チケット作成API仕様（ベースAPIとの整合性含む） |
